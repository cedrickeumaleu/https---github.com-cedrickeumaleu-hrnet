  ...
  const endIndex = Math.min(startIndex + perPage - 1, filteredEmployees.length);
   ...
   const renderPageButtons = () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      buttons.push(
        <button key={i} variant="secondary" onClick={() => goToPage(i)}>{i}</button>
      );
    }
    return buttons;
  };
  ...
  return (
    ...
    <Form.Group controlId="searchForm">
      ...
      <div className='footertable'>
        <div>Showing {startIndex} to {endIndex} of {filteredEmployees.length} entries</div>
      
        <div className='currentpage'>
          {/* Bouton "Précédent" */}
          <button variant="secondary" onClick={goToPreviousPage} disabled={currentPage === 1}>Previous</button>
          {' '}
          ...
          {renderPageButtons()}
          {/* Bouton "Suivant" */}
          <button variant="secondary" onClick={goToNextPage} disabled={currentPage === totalPages}>Next</button>
        </div>
      </div>
      <a href="/">Home</a>
    </div>
